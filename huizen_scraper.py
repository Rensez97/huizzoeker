    ...
    try:
        for i in range(10):
            ...
            for huis in huizen:
                try:
                    # #searching for status on home page for availability and skip iteration if unavailable
                    begin_status = huis.find("div", {"class": "object_status_container"}).text.strip()
                    status = ' '.join(begin_status.split())
                    # status = huis.find("span", {"class": "object_status rented"}).text.strip()
                    if "Verhuurd" in status:
                        continue
...
def bensverhuur():
    ...
    try:
        ...
        email_error(bensverhuur.__name__, e2)
        print("Connection error")
